...
import EditBoxType1 from '../EditBox/EditBoxType1';
import EditBoxType2 from '../EditBox/EditBoxType2';
import {hombeBoxItemInfo,homeboxItemDialogEdit} from '../../actions/actionsHomeBoxItems';


class HandlerBoxEditor extends Component {
  constructor(props) {
      super(props);
    ...
    hanleEditor(){
      ...
      dispatch(hombeBoxItemInfo({idEdition: (item.id)?item.id:''}));
      dispatch(hombeBoxItemInfo({typeBox: (item.typeBox)?item.typeBox:''}));
      dispatch(hombeBoxItemInfo({typeIcon: (item.typeIcon)?item.typeIcon:''}));
      dispatch(hombeBoxItemInfo({urlImg: (item.urlImg)?item.urlImg:''}));
      dispatch(hombeBoxItemInfo({urlImg1: (item.urlImg1)?item.urlImg1:''}));
      dispatch(hombeBoxItemInfo({urlImg2: (item.urlImg2)?item.urlImg2:''}));
      dispatch(hombeBoxItemInfo({urlImg3: (item.urlImg3)?item.urlImg3:''}));
      dispatch(hombeBoxItemInfo({titulo: (item.titulo)?item.titulo:''}));
  ...
  //  console.log("TIPO BOx: "+this.props.item.typeBox);
    ...
    return(
        <div className="handler-box-editor editor_off">
          ...
          <Dialog  open={homeBoxItemEditorReducer.isOpenDialogEdit} style={{width:'auto'}}>
         <h4>Modo Editor</h4>
         <DialogContent>
           ...
           }
           {
            ...
            { /*(this.props.item)?
                ((this.props.item.typeBox==="tpb1")? <EditBoxType1 item={this.props.item}/> : <div/> ) : <div/>

            }


            { (this.props.item)?
                ((this.props.item.typeBox==="tpb2")? <EditBoxType2 item={this.props.item}/> : <div/> ) : <div/> */  }
